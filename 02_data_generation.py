...
def data_generation(paths):
    ...
    group_by_id_df['word_length'] = group_by_id_df['word_length'].replace([np.inf, -np.inf], 0)
    ...
    for column in df.columns:
        if column not in ['id', 'event_id', 'up_time', 'down_time', 'action_time', 'cursor_position', 'word_count']:
            ...
            group_df['word_length'] = group_df['word_length'].replace([np.inf, -np.inf], 0)
...
if __name__ == '__main__':
    for index in range(9):
        path = f'datasets/train_logs_{index}.csv'
